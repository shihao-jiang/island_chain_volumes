...
def IHotVol_GravForward(grdfile, denangrdfile, edificegrdfile,
    ...
    if subaq == 0:
        ...
        run(f'gmt grdmath {subaerialgrdfile} 2400 MUL = subaerial_rho.grd')
        ...
        run(f'gmt grdmath {denangrdfile} {subaerialgrdfile} MUL = subaerialtopo.grd')
        run('gmt grdmath subaerialtopo.grd 0 LE gravmodel/sed.grav.grd MUL = gravmodel/sed.grav.grd')

    if subaq == 1:
        run(f'gmt grdmath {denangrdfile} {sedcutgrdfile} SUB = gravmodel/rockoffset.grd')
    ...
    run(f'gmt grdmath {mohoflexgrdfile} 12000 SUB = {mohoflexgrdfile}')
    run((f'gravfft {mohoflexgrdfile} -D{rho_m - rho_c} '
         f'-E4 -fg -N+a -Ggravmodel/flex.moho.grav.grd'))

    # Get age and regional bathymetry
    run(f'gmt grdsample cutagetmp/age.{grdfile} -R{grdfile}_regional.grd '
    ...
    for iso_T in range(50, 600, 50):   # 50, 100, ..., 550
        ...
        run(f'gmt grdmath gravmodel/depth.{iso_T}C.grd 0 DENAN = gravmodel/depth.{iso_T}C.grd')
        ...
        if iso_T == 50:
            ...
            run('gmt grdmath gravmodel/Grav.tmp.grd gravmodel/thermal.grav.grd ADD = gravmodel/thermal.grav.grd')

    # Resize grids to common extent
    run('gmt grdsample -Rgravmodel/thermal.grav.grd gravmodel/flex.moho.grav.grd '
        '-Ggravmodel/flex.moho.trim.grav.grd')
    run('gmt grdsample -Rgravmodel/thermal.grav.grd gravmodel/rock.grav.grd '
        '-Ggravmodel/rock.trim.grav.grd')
    run('gmt grdsample -Rgravmodel/thermal.grav.grd gravmodel/sed.grav.grd '
